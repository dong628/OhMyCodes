...
const int Maxn=205;
int n, m, mapp[Maxn][Maxn], dp[Maxn*2][Maxn][Maxn], dp1[Maxn*2][Maxn][Maxn], kr, lp;
...
//	freopen("data.in", "r", stdin);
	...
	for(int i=1; i<=n; i++){
		for(int j=1; j<=m; j++){
			scanf("%d", &mapp[i][j]);
			minn = minn<mapp[i][j]?minn:mapp[i][j];
			maxx = maxx>mapp[i][j]?maxx:mapp[i][j];
		}
	}
	l=minn; r=maxx;
//	if((2*(n+m)-4)&1){
...
//	return dp[n+m-3][n][n-1];
	if(dp[n+m-3][n][n-1]>=(2*(n+m)-4)/2+kr){
		return true;
	}
	return false;
}
